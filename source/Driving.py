...
#Controller Class Takes in the x,y, and radius pixels and determines the PWM
#to send to the ESC.
class Controller(object):
    ...
    def __init__(self):
        ...
        self.startDonuts=False
        
        
    def run(self, x,y,Current_radius):
        #print('y dist:',y)
        if x is not None: #If we found at least 1 contour
            self.startDonuts=False #Dont do donuts
            print('found something')
            if y <=cfg2.y_Limit_Current: #If the contour isn't too close
                if Current_radius >cfg2.min_Pixel_Radius:
                    print('found something sigificant')
                    self.donutReset=True
                    ...
                    if cfg2.Steering_Control_Option: #If implementing Control to Steering
                        ...
                        self.PWM_Steering=cfg2.m_Steering*self.theta_center+cfg2.b_Steering; #What PWM value do we need to turn correctly  
                    ...
                    self.PWM_Throttle=self.WithinBounds(Value,cfg2.PWMHigh_Throttle,cfg2.PWMLow_Throttle)
                    return self.PWM_Steering, self.PWM_Throttle          
                    
                else:
                    print('Contour Too Small')
                    self.startDonuts=True
            else:
                print('Has Ball')
                cfg2.currentColor_lower=cfg2.secondColor_lower #Start looking for second color (base)
                cfg2.currentColor_upper=cfg2.secondColor_upper
                cfg2.y_Limit_Current=cfg2.y_Limit2
                self.startDonuts=True
        ...
        if self.startDonuts==True:
            if self.donutReset:# Waits 3 seconds before starting donuts
                print('starting donut time')
                self.donutReset=False
    ...
    def shutdown(self):
        ...
        cfg2.currentColor_lower=cfg2.green_lower
        cfg2.currentColor_upper=cfg2.green_upper
        self.running=False
